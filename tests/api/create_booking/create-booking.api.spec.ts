...
test.describe('[Booking API] Create Booking', () => {
  ...
  test('CREATE_BOOKING_ID_001 - should create booking successfully with valid required and optional fields', async ({
    ...
    expect(body.booking.bookingdates.checkin).toBe(createBookingData.bookingdates.checkin);
    expect(body.booking.bookingdates.checkout).toBe(createBookingData.bookingdates.checkout);
    expect(body.booking.additionalneeds).toBe(createBookingData.additionalneeds);

    // Step 9: Log test result
    logger.pass(`CREATE_BOOKING_ID_001 - Booking created successfully with ID: ${body.bookingid}`);
  });
  ...
  test('CREATE_BOOKING_ID_002 - should display created booking id in get booking ids response', async ({
    ...
    await severity('critical');
    await description('Verify that a newly created booking appears in GET /booking response list.');
    ...
    await test.step('Create booking and get booking list', async () => {
      ...
      const foundBooking = getBody.find((booking) => booking.bookingid === newBookingId);

      expect(foundBooking).toBeDefined();
      expect(foundBooking?.bookingid).toBe(newBookingId);

      logger.info(`Created booking ID ${newBookingId} found in GET /booking response`);
